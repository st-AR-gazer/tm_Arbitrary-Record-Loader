...
namespace LRBasedOnCurrentMap {
    ...
    void RTPart_ValidationReplay() {
        ...
        if (ValidationReplay::ValidationReplayExists()) {
            ...
            int vrTime = ValidationReplay::GetValidationReplayTime();
            if (vrTime > 0) {
    ...
    class MedalUIEntry {
        ...
        MedalUIEntry() {}
        MedalUIEntry(const string &in _label, const string &in _color, Medals::Medal@ _medal, bool _depPresent) {
            label = _label;
            colorCode = _color;
            @medal = _medal;
    ...
    void InitMedalEntries() {
        ...
        hasSBVille = true;
#endif
        medalEntries.InsertLast(MedalUIEntry("Champion",  "\\$e79", champMedal,   hasChamp));
        medalEntries.InsertLast(MedalUIEntry("Warrior",   "\\$0cf", warriorMedal, hasWarrior));
        medalEntries.InsertLast(MedalUIEntry("SB Ville",  "\\$f90", sbVilleMedal, hasSBVille));
        medalEntries.InsertLast(MedalUIEntry("Author",    "\\$7e0", authorMedal,  true));
        medalEntries.InsertLast(MedalUIEntry("Gold",      "\\$fd0", goldMedal,    true));
        medalEntries.InsertLast(MedalUIEntry("Silver",    "\\$ddd", silverMedal,  true));
        medalEntries.InsertLast(MedalUIEntry("Bronze",    "\\$c73", bronzeMedal,  true));
    }
        ...
        if (UI::BeginTable("ARL_Medals", 5, flags)) {
            ...
            for (uint i = 0; i < medalEntries.Length; i++) {
                ...
                if (e.medal.medalExists)
                    UI::Text(FromMsToFormat(e.medal.currentMapMedalTime));
                else
                    UI::Text("-");
