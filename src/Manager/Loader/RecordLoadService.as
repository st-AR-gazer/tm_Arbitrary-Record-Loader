namespace RecordLoadService {
    ...
    class RecordLoadRequest {
        ...
        LoadedRecords::SourceKind sourceKind = LoadedRecords::SourceKind::MapRecord;
        string sourceRef = "";
    ...
    int Enqueue(RecordLoadRequest@ req) {
        ...
        log("Queued record load job #" + job.id + " for mapUid=" + req.mapUid + ", rankOffset=" + req.rankOffset + ", saveLocation=" + req.saveLocation, LogLevel::Info, 69, "RecordLoadService");
    ...
    void LogMapInfoCandidates(const string &in mapUid, const string &in providedMapId, const array<MapInfoCandidate@> &in candidates) {
        ...
        if (parts.Length == 0) {
            log("No mapId candidates resolved for mapUid=" + mapUid, LogLevel::Warn, 169, "RecordLoadService");
            return;
        }
        log("Resolved mapId candidates for mapUid=" + mapUid + ": " + JoinStrings(parts), LogLevel::Info, 172, "RecordLoadService");
    }

    string DefaultSourceRef(RecordLoadRequest@ req, const string &in accountId = "") {
        if (req is null) return "";
        string source = req.saveLocation.Length > 0 ? req.saveLocation : "Record";
        string ref = source + " | " + req.mapUid + " | #" + tostring(req.rankOffset + 1);
        if (req.saveLocation == "PlayerId" && accountId.Length > 0) {
            ref += " | " + accountId;
        }
        return ref;
    }
    ...
    void WorkerLoop() {
        while (g_Queue.Length > 0) {
            ...
            if (job.state == JobState::Failed) {
                ...
                NotifyWarn("Record load failed: " + notifyMsg);
            } else if (job.state == JobState::Cancelled) {
                log("Record load job cancelled #" + job.id, LogLevel::Warn, 145, "RecordLoadService");
            }
    ...
    void ProcessJob(RecordLoadJob@ job) {
        ...
        // Nadeo uses two different identifiers here:
        // - live leaderboard lookup: mapUid -> accountId
        // - core replay lookup: mapId + accountId -> replay url
        // The core mapRecords API does not accept mapUid, so we must translate first.
        ...
        if (accountId.Length == 0) {
            log("Resolving accountId for job #" + job.id + " via leaderboard", LogLevel::Info, 189, "RecordLoadService");
        ...
        if (wsid.Length > 0) {
            if (!IsScoreMgrReady()) {
                ...
                log("ScoreMgr backend not ready for job #" + job.id + "; waiting up to " + REPLAY_URL_BACKEND_WAIT_MS + " ms before HTTP fallback", LogLevel::Warn, 208, "RecordLoadService");
                bool becameReady = WaitForScoreMgrReady(REPLAY_URL_BACKEND_WAIT_MS);
                if (becameReady) {
                    gameBackendUnavailable = false;
                    log("ScoreMgr backend became ready for job #" + job.id + "; retrying in-game replay-url resolution", LogLevel::Info, 209, "RecordLoadService");
                }
            }
        ...
        if (replayUrl.Length == 0) {
            log("Falling back to HTTP replay-url resolution for job #" + job.id, LogLevel::Warn, 210, "RecordLoadService");
        ...
        if (!cacheOk || doRefresh) {
            log("Downloading record file for job #" + job.id + " to cache: " + cachePath, LogLevel::Info, 222, "RecordLoadService");
            ...
            log("Using cached record file for job #" + job.id + ": " + cachePath, LogLevel::Info, 231, "RecordLoadService");
        }
        ...
        log("Dispatching cached file to local loader for job #" + job.id + ": " + cachePath, LogLevel::Info, 245, "RecordLoadService");
        loadRecord.LoadRecordFromLocalFile(cachePath);
    }

    string CachePathFor(RecordLoadRequest@ req, const string &in accountId) {
        if (req is null) return "";
        ...
        string loc = req.saveLocation;
        if (loc == "Official") {
            baseDir = Server::officialFilesDirectory;
            prefix = "Official";
            ext = ".Ghost.Gbx";
        } else if (loc == "GPS") {
            baseDir = Server::currentMapRecordsGPS;
            prefix = "GPS";
            ext = ".Replay.Gbx";
        } else if (loc == "AnyMap") {
            ...
            baseDir = Server::savedFilesDirectory;
            prefix = "AutoMove";
            ext = ".Replay.Gbx";
        }
    ...
    LoadedRecords::SourceKind DefaultSourceKind(const RecordLoadRequest@ req) {
        if (req is null) return LoadedRecords::SourceKind::MapRecord;
        if (req.saveLocation == "Official") return LoadedRecords::SourceKind::Official;
        if (req.saveLocation == "OtherMaps") return LoadedRecords::SourceKind::Profile;
        if (req.saveLocation == "PlayerId") return LoadedRecords::SourceKind::PlayerId;
    ...
    string ResolveWebServicesUserId(const string &in accountId) {
        ...
        try {
            ...
            log("Failed to resolve web services user id from accountId=" + accountId, LogLevel::Warn, 327, "RecordLoadService");
        }
        return "";
    }
    ...
    string ResolveReplayUrlFromScoreMgr(const string &in wsid, const string &in mapUid, const string &in gameMode = "TimeAttack") {
        ...
        if (!resp.HasFailed && resp.HasSucceeded && resp.MapRecordList.Length > 0) {
            ...
            log("ScoreMgr replay-url lookup returned no records for mapUid=" + mapUid + ", wsid=" + wsid + ", gameMode=" + FormatGameModeLabel(gameMode), LogLevel::Warn, 328, "RecordLoadService");
        }
    ...
    string ResolveReplayUrlFromPlayground(const string &in wsid, const string &in mapUid, const string &in gameMode = "TimeAttack") {
        auto ps = cast<CSmArenaRulesMode>(GetApp().PlaygroundScript);
        if (ps is null) {
            log("Playground replay-url lookup unavailable: playground script is null for mapUid=" + mapUid, LogLevel::Warn, 362, "RecordLoadService");
            return "";
        }
        if (ps.UserMgr is null) {
            log("Playground replay-url lookup unavailable: UserMgr is null for mapUid=" + mapUid, LogLevel::Warn, 365, "RecordLoadService");
            return "";
        }
        if (ps.UserMgr.Users.Length == 0) {
            log("Playground replay-url lookup unavailable: no local users for mapUid=" + mapUid, LogLevel::Warn, 368, "RecordLoadService");
            return "";
        }
        ...
        if (resp is null) {
            log("Playground replay-url lookup returned null task for mapUid=" + mapUid + ", wsid=" + wsid + ", gameMode=" + FormatGameModeLabel(gameMode), LogLevel::Warn, 374, "RecordLoadService");
            return "";
        }
        ...
        if (!resp.HasFailed && resp.HasSucceeded && resp.MapRecordList.Length > 0) {
            ...
            log("Playground replay-url lookup returned no records for mapUid=" + mapUid + ", wsid=" + wsid + ", gameMode=" + FormatGameModeLabel(gameMode) + ", failed=" + tostring(resp.HasFailed) + ", succeeded=" + tostring(resp.HasSucceeded), LogLevel::Warn, 380, "RecordLoadService");
        }
    ...
    string ResolveReplayUrlFallback(const string &in mapUid, const string &in accountId, const string &in providedMapId, const string &in seasonId, const array<MapInfoCandidate@> &in resolvedCandidates) {
        ...
        if (mapIds.Length == 0) {
            log("HTTP replay-url fallback could not resolve mapId for mapUid=" + mapUid, LogLevel::Warn, 356, "RecordLoadService");
            return "";
        }
    ...
    Json::Value ResolveMapInfoFromUrl(const string &in mapUid, const string &in url, const string &in routeLabel = "") {
        ...
        if (req.ResponseCode() != 200) {
            ...
            log("ResolveMapInfo" + routeSuffix + " failed for mapUid=" + mapUid + ", code=" + req.ResponseCode(), LogLevel::Warn, 424, "RecordLoadService");
            return Json::Object();
        }
        ...
        if (data.GetType() != Json::Type::Array || data.Length == 0) {
            string routeSuffix = routeLabel.Length > 0 ? " (" + routeLabel + ")" : "";
            log("ResolveMapInfo" + routeSuffix + " returned no map data for mapUid=" + mapUid, LogLevel::Warn, 429, "RecordLoadService");
            return Json::Object();
        }
        return data[0];
    }
    ...
    string TryResolveReplayUrlFromCoreEndpoint(const string &in routeLabel, const string &in url, const string &in mapUid, const string &in accountId, const string &in mapId) {
        ...
        if (req.ResponseCode() != 200) {
            log("HTTP replay-url fallback (" + routeLabel + ") failed for mapUid=" + mapUid + ", accountId=" + accountId + ", code=" + req.ResponseCode() + ", url=" + url + ", body=" + CompactForLog(responseText), LogLevel::Warn, 452, "RecordLoadService");
            return "";
        }
        ...
        if (data.GetType() == Json::Type::Object) {
            ...
            log("HTTP replay-url fallback (" + routeLabel + ") returned error for mapUid=" + mapUid + ", accountId=" + accountId + ", mapId=" + mapId + ": " + code + " " + message + ", url=" + url + ", body=" + CompactForLog(responseText), LogLevel::Warn, 459, "RecordLoadService");
            return "";
        }
        if (data.GetType() != Json::Type::Array || data.Length == 0) {
            log("HTTP replay-url fallback (" + routeLabel + ") returned no records for mapUid=" + mapUid + ", accountId=" + accountId + ", mapId=" + mapId + ", url=" + url + ", body=" + CompactForLog(responseText), LogLevel::Warn, 463, "RecordLoadService");
            return "";
        }

        string replayUrl = string(data[0]["url"]);
        if (replayUrl.Length == 0) {
            log("HTTP replay-url fallback (" + routeLabel + ") returned a record without a url for mapUid=" + mapUid + ", accountId=" + accountId + ", mapId=" + mapId + ", url=" + url + ", body=" + CompactForLog(responseText), LogLevel::Warn, 468, "RecordLoadService");
            return "";
        }
        return replayUrl;
    }
